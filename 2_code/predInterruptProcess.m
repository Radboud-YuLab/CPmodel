function pred_exp = predInterruptProcess(getOptCPoutput, predTime, timeCutoff)

% calculates the predicted effect (on gene expression) of interrupting a 
...
% predTime         the time(s) at which prediction is desired. An array.
% timeCutoff       the time between two states where there is an
%                  interruption. I.e. the previous state can complete, the
%                  next state not.
